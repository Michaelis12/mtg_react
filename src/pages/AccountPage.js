...
const AccountPage = () => {
    ...
    const [userLikes, setUserLikes] = useState(0) // Mieux de l'initialiser à 0
    ...
    const fetchCsrfToken = async () => {
        ...
        // setDisplayLoading(false); // Commenté car setDisplayLoading est appelé dans finally
        } catch (err) {
...
                        setPageCardLiked(1)
                        // Si response.data est undefined ou null, cette ligne était potentiellement dangereuse
...
        }
        getDeckBuilder();
        // L'effet secondaire dépend de ces changements, mais ils peuvent être omis si la fonction est bien gérée
        // pour éviter les boucles, mais dans ce cas, nous les laissons comme ils étaient.
...
                    try {
                        setDisplayLoading(true)
                        // --- MODIFICATION 2 : Chaînage optionnel sur deckBuilder.id ---
                        if (deckBuilder?.id) { 
...
                }
            getUserLikes();
            // Ajout de deckBuilder.id aux dépendances si on veut que ça se re-déclenche après le premier chargement
            // L'erreur venait probablement de là si getDeckBuilder n'était pas assez rapide
            }, [deckBuilder?.id]); 
...
        const displayAvatar = () => { 
            // --- MODIFICATION 3 : Vérification de deckBuilder au début de la fonction ---
            if (!deckBuilder) {
                return ''; // Retourne une chaîne vide en attendant le chargement
            }
                if(avatar === "") {
...
                }
// ... (suite de la fonction displayAvatar sans changement)


// ... (autres fonctions et styles sans changement)
...
                            )}
                    ...
                    {/* --- NOUVEAU : Bloc de chargement en attendant deckBuilder --- */}
                    { !deckBuilder && !displayLoading && (
                        <div className='p-blank-section' style={{ minHeight: '500px', backgroundImage: `url(${BackgroundWhite})`, display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
                            <ParagraphBlank text={"Chargement du profil..."}/>
                        </div>
                    )}
                    {/* --- FIN NOUVEAU : Bloc de chargement --- */}

                    {/* --- Le reste du contenu n'est affiché que si deckBuilder est chargé et que le profil n'est pas inactif --- */}
                    { deckBuilder && !activeProfil && (
                        <> 
                            { activeProfil && (
                                <div className='popup-bckg'>
                                    {/* ... Contenu de la popup d'activation ... */}
                                </div>  
                            )}  
                            ...
                            <div className="card-user-container">                    
                                    ...
                                    <div className="card-user-desktop" style={{ backgroundImage: `url(${BackgroundWhite})`}}>
                                        ...
                                        <div className="card-user-info">
                                            ...
                                            <div className="card-line-attribut">
                                                ...
                                                {/* --- MODIFICATION 3 : Accès sécurisé à deckBuilder.dateSign --- */}
                                            ...
                                            <div className='user-stat-container'>
                                                ...
                                                <div className='user-stat'>
                                                    <h5>Decks créés : </h5>
                                                    {/* --- MODIFICATION 3 : Accès sécurisé à deckBuilder.decksNumber --- */}
                                                    <h4>{deckBuilder.decksNumber || 0}</h4>
                                                </div>
                                                <div  className='user-stat'>
                                                    <h5>Likes obtenus : </h5>
                                                    <h4>{userLikes}</h4>
                                    ...
                                    {/* Version mobile de la carte utilisateur (sécurisée de la même manière) */}
                                    ...
                                    <div className='popup-bckg'>
                                        <div className="card-user-desktop" style={{ backgroundImage: `url(${BackgroundWhite})`}}>
                                            ...
                                            <div className="card-user-info">
                                                {/* --- MODIFICATION 3 : Accès sécurisé dans le textarea (avec || "") --- */}
                                                <textarea className="user-pseudo" id="pseudo" name="pseudo" rows="1"
                                                    style={{color: 'black', backgroundColor: 'white'}}
                                                    maxLength={25} onChange={(e) => setPseudo(e.target.value)} defaultValue={deckBuilder?.pseudo || ""} >
                                                </textarea>
                                                ...
                                                <textarea className="input-bio" id="bio" name="bio" rows="4"
                                                    maxLength={100} onChange={(e) => setBio(e.target.value)} defaultValue={deckBuilder?.bio || ""} >
                                                </textarea>
                                        ...
                                        {/* Version mobile du formulaire (sécurisée de la même manière) */}
                                        <div className="card-user-mobile"> 
                                                <div className="header-card" style={{backgroundImage:`url(${BackgroundGif})`}}>
                                                    <img src={displayAvatar()} className="user-avatar" alt="user-pp"/>
                                                    <div className='input-image-container'>
                            ...
                            <div className="user-attributs-container" style={{marginTop: '3%'}}>
                                ...
                                <div className="open-buttons-container">
                                    ...
                                    <div style={{width: '100%'}}>
                                        {/* deckBuilder?.decksNumber est déjà sécurisé */}
                                    ...
                                    {/*Mapping des cartes likées (commenté dans le code original, donc pas de changement nécessaire)*/}

                                    
                                    {/*Mapping des decks likés*/}
                                    <div style={{width: '100%'}} className='decks-liked-map-contenair'>
                                        {/* deckBuilder?.decksLikedNumber est déjà sécurisé */}
